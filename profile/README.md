# InvoiceCN

**Beautiful invoices, built from composable components.**

A developer-focused registry of invoice components, document blocks, themes, calculation primitives, and complete templates — install what you need, compose declaratively, render everywhere:

Web & customer portals · PDF · A4 / US Letter print · email-safe HTML · mobile · PNG previews · thermal receipts · structured e-invoice formats · accessible archives

## 🔒 Early access is sponsor-only

The source lives in a private repository during early access. Two ways in:

- **[Sponsor InvoiceCN](https://github.com/sponsors/InvoiceCN)** → instant access to the private repo, plus a vote on what ships next
- **Wait** for the public release

## 🗺️ Steer the roadmap

Sponsors get access to roadmap discussions and priority on feature requests. Public discussions live in [invoicecn](https://github.com/InvoiceCN/invoicecn).
